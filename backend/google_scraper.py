...
try:
    ...
    from base.models import Product, Service, ServiceImage, Review  # ← Change 'base' to your app name
...
# Counter for caterer usernames: caterer1, caterer2, ...
def get_next_caterer_index():
    """Find the next available caterer index."""
    if not DJANGO_AVAILABLE:
        return 1
    i = 1
    while User.objects.filter(username=f"caterer{i}").exists():
        i += 1
    return i
...
def create_caterer_user(index):
    """Create a unique user: caterer1, caterer2, caterer3..."""
    if not DJANGO_AVAILABLE:
        return None
    username = f"caterer{index}"
    user = User.objects.create(
        username  = username,
        email     = f"{username}@bookyourcelebration.com",
        first_name = "Caterer",
        last_name  = str(index),
    )
...
def download_image(url, filename):
    """Download image from URL, return (ContentFile, filename)."""
...
def save_to_db(business, caterer_index, details, dry_run=False):
    """
    Save one business to Django DB:
      - Creates unique user: caterer1, caterer2...
      - Saves Product with image
      - Saves Services with Google rating + 2 images each
      - Saves Google reviews into Review model
    """
    ...
    if dry_run:
        log.info(f"\n  [DRY RUN] ✅ {name}")
        log.info(f"    👤 User     : caterer{caterer_index}")
        ...
        log.info(f"    📸 Photos  : {len(photos)} available for service images")
    ...
    try:
        ...
        # ── Create unique caterer user ──────────────────────────────
        caterer_user = create_caterer_user(caterer_index)
        ...
        # ── Product main image (photos[0]) ──────────────────────────
        image_url = business.get("image_url", "")
        if image_url:
            content, filename = download_image(image_url, f"{name[:40].replace(' ', '_')}.jpg")
            if content:
                product.image.save(filename, content, save=False)
                log.info(f"    🖼️  Product image saved: {filename}")

        product.save()
        log.info(f"  ✅ Saved Product: {name}  (id={product._id}, user=caterer{caterer_index})")

        # ── Service images pool: photos[1:] (skip product cover photo) ──
        photo_refs = [p.get("photo_reference", "") for p in photos[1:] if p.get("photo_reference")]

        # ── Create Services ─────────────────────────────────────────
        for svc_idx, svc in enumerate(business.get("services", [])):
            ...
            log.info(f"    🍽️  Service: {service.name}  ₹{service.price}  ⭐{service.rating}")
            ...
            for ref in photo_refs[:]:
                ...
                fname     = f"svc_{product._id}_{svc_idx}_{images_saved}.jpg"
                content, filename = download_image(photo_url, fname)
                if content:
                    try:
                        svc_img = ServiceImage(service=service)
                        svc_img.image.save(filename, content, save=True)
                        log.info(f"       📸 Service image {images_saved+1}: {filename}")
                        images_saved += 1
                        photo_refs.pop(0)  # use each photo only once
                    except Exception as e:
                        log.warning(f"       ⚠️  ServiceImage save failed: {e}")

        # ── Save Google Reviews → Review model ──────────────────────
        first_service = product.services.first()
        reviews_saved = 0
        for rev in reviews[:5]:  # max 5 reviews
            rev_text   = rev.get("text", "").strip()
            rev_rating = rev.get("rating", 3)
            rev_name   = rev.get("author_name", "Google User")
            if not rev_text or not first_service:
                continue
            ...
            reviews_saved += 1
            log.info(f"    💬 Review: {rev_name} ⭐{rev_rating}")

        if reviews_saved:
            log.info(f"    💬 Total reviews saved: {reviews_saved}")
...
def run(dry_run=False, limit=None):
    ...
    # Start caterer index from next available number
    caterer_index = get_next_caterer_index()
    ...
    for i, place in enumerate(all_places, 1):
        ...
        business = {
            ...
            "photos":         photos,        # all photo refs for service images
            "services":       services,
        }

        # Save to DB — create unique caterer user for each product
        result = save_to_db(business, caterer_index, details, dry_run=dry_run)
        if result:
            saved += 1
            caterer_index += 1
        else:
